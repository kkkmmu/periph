...
func TestI2C_Draw_VerticalLSD_fast(t *testing.T) {
	...
	img.Pix[22] = 1
	if err := dev.Draw(img, image.Point{}); err != nil {
		t.Fatal(err)
	}
	if err := bus.Close(); err != nil {
		t.Fatal(err)
...
func TestI2C_Draw_fail(t *testing.T) {
	...
	if err := dev.Draw(makeGrayCheckboard(dev.Bounds()), image.Point{}); !conntest.IsErr(err) {
		t.Fatalf("expected conntest error: %v", err)
	}
	if err := bus.Close(); err != nil {
		t.Fatal(err)
...
func TestI2C_DrawGray(t *testing.T) {
	...
	if err := dev.Draw(makeGrayCheckboard(dev.Bounds()), image.Point{}); err != nil {
		t.Fatal(err)
	}
	// No-op (skip path).
	if err := dev.Draw(makeGrayCheckboard(dev.Bounds()), image.Point{}); err != nil {
