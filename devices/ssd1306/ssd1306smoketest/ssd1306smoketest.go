...
func (s *SmokeTest) run(i2cBus i2c.Bus, spiPort spi.PortCloser, dc gpio.PinOut, opts *ssd1306.Opts) (err error) {
	...
	for i, d := range s.devices {
		start := time.Now()
		if err := d.Draw(imgBunnyNRGBA, image.Point{}); err != nil {
			return err
		}
		s.timings[i] = time.Since(start)
	}
	...
	for i, d := range s.devices {
		start := time.Now()
		if err := d.Draw(imgBunny1bitLarge, image.Point{}); err != nil {
			return err
		}
		s.timings[i] = time.Since(start)
	}
	...
	for i, d := range s.devices {
		start := time.Now()
		if err := d.Draw(imgBunny1bit, image.Point{}); err != nil {
			return err
		}
		s.timings[i] = time.Since(start)
	}
	...
	for i, d := range s.devices {
		start := time.Now()
		if err := d.Draw(bmp, image.Point{}); err != nil {
			return err
		}
		s.timings[i] = time.Since(start)
	}
